...
TorchRgb8Image *inferNafnet(const TorchRgb8Image *input,
                            const char *modelPath) {
  return Nafnet().infer(input, modelPath);
}

namespace {

TorchRgb8Image *Nafnet::infer(const TorchRgb8Image *input,
                              const char *modelPath) {
  ...
  try {
    ...
    const unsigned padX = (16 - input->width % 16) % 16;
    const unsigned padY = (16 - input->height % 16) % 16;
    padded = input;
    if (padX > 0 || padY > 0) {
