...
class MotionDeblur implements RgbMethod {
  ...
  Future<void> cleanResource() {
    return _modelDownloader.delete(ModelType.nafnetGopro);
  }

  @override
  Future<bool> isResourceReady() {
    return _modelDownloader.isDownloaded(ModelType.nafnetGopro);
  }
  ...
  Future<Rgb8Image?> apply(Rgb8Image src) async {
    final modelFile = await ModelDownloader().download(ModelType.nafnetGopro);
  ...
  static Future<Rgb8Image?> _applyAsync(
    ...
    return Isolate.run(() async {
      return await inferNafnet(src, modelPath: modelFile.path);
    });
  }
